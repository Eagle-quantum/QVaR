...
class ParallelIterativeAmplitudeEstimation(AmplitudeEstimator):
    ...
    def construct_circuit(
        ...
        circuit = QuantumCircuit(num_qubits*self._l+self._l, name="parallel circuit")
        ...
        for i in range(0, self._l):
            circuit.compose(estimation_problem.state_preparation,
                            qubits=circuit.qubits[1+i+(i*estimation_problem.state_preparation.num_qubits):1+i+((i+1)*estimation_problem.state_preparation.num_qubits)], 
                            inplace=True)

        for i in range(1, self._l):
            circuit.cx(circuit.qubits[0], circuit.qubits[i+i*estimation_problem.state_preparation.num_qubits])

        # add Q^k
        if k % self._l == 0:
            # circuit.compose(estimation_problem.grover_operator.power(int(k/self._l)).control(), inplace=True)
            for i in range(0, self._l):
                circuit.compose(estimation_problem.grover_operator.power(int(k/self._l)).control(), 
                                qubits=circuit.qubits[i+(i*estimation_problem.state_preparation.num_qubits):i+1+((i+1)*estimation_problem.state_preparation.num_qubits)], 
                                inplace=True)
        else:
            circuit.compose(estimation_problem.grover_operator.power(int(k/self._l)+1).control(), inplace=True)
            for i in range(1, self._l):
                circuit.compose(estimation_problem.grover_operator.power(int(k/self._l)).control(), 
                                qubits=circuit.qubits[i+(i*estimation_problem.state_preparation.num_qubits):i+1+((i+1)*estimation_problem.state_preparation.num_qubits)], 
                                inplace=True)
        ...
        if measurement:
            ...
            circuit.barrier()
            circuit.measure(circuit.qubits[0], c[:])
    ...
    def estimate(
        ...
        if self._quantum_instance.is_statevector:
            ...
            while theta_intervals[-1][1] - theta_intervals[-1][0] > self._epsilon / np.pi:
                ...
                circuit = self.construct_circuit(estimation_problem, 2*k+1, measurement=True)
                ret = self._quantum_instance.execute(circuit)
